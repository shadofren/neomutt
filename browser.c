                             ...
                             const char *d, const char *prefix)
{
  ...
  if (OptNews)
  {
    ...
    for (unsigned int i = 0; i < adata->groups_num; i++)
    {
      ...
      if (C_Mask && C_Mask->regex &&
          !((regexec(C_Mask->regex, mdata->group, 0, NULL, 0) == 0) ^ C_Mask->not_))
      {
        continue;
      }
...
#endif /* USE_NNTP */
  {
    ...
    while ((de = readdir(dp)))
    {
      ...
      if (C_Mask && C_Mask->regex &&
          !((regexec(C_Mask->regex, de->d_name, 0, NULL, 0) == 0) ^ C_Mask->not_))
      {
        continue;
      }
...
static int examine_mailboxes(struct Menu *menu, struct BrowserState *state)
{
  ...
  if (OptNews)
  {
    ...
    {
      struct NntpMboxData *mdata = adata->groups_list[i];
      if (mdata && (mdata->new_ || (mdata->subscribed && (mdata->unread || !C_ShowOnlyUnread))))
      {
        add_folder(menu, state, mdata->group, NULL, NULL, NULL, mdata);
      }
    }
