                                   ...
                                   unsigned long data, struct Buffer *err)
{
  ...
  TAILQ_FOREACH(ptr, &Hooks, entries)
  {
    ...
    else if ((ptr->type == data) && (ptr->regex.not_ == not_) &&
...
void mutt_folder_hook(const char *path, const char *desc)
{
  ...
  TAILQ_FOREACH(tmp, &Hooks, entries)
  {
    ...
    if ((path && (regexec(tmp->regex.regex, path, 0, NULL, 0) == 0) ^ tmp->regex.not_) ||
        (desc && (regexec(tmp->regex.regex, desc, 0, NULL, 0) == 0) ^ tmp->regex.not_))
    {
...
void mutt_account_hook(const char *url)
{
  ...
  TAILQ_FOREACH(hook, &Hooks, entries)
  {
    ...
    if ((regexec(hook->regex.regex, url, 0, NULL, 0) == 0) ^ hook->regex.not_)
    {
      inhook = true;
...
const char *mutt_idxfmt_hook(const char *name, struct Mailbox *m, struct Email *e)
{
  ...
  TAILQ_FOREACH(hook, hl, entries)
  {
    ...
    if ((mutt_pattern_exec(pat, 0, m, e, &cache) > 0) ^ hook->regex.not_)
    {
