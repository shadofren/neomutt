 ...
 */
int mutt_save_message(struct Mailbox *m, struct EmailList *el, bool delete_original,
                      bool decode, bool decrypt)
{
  ...
  if (single)
  {
    if (mutt_save_message_ctx(en->email, delete_original, decode, decrypt, ctx_save->mailbox) != 0)
    {
  ...
  else
  {
    ...
    {
      mutt_message_hook(m, en->email, MUTT_MESSAGE_HOOK);
      rc = mutt_save_message_ctx(en->email, delete_original, decode, decrypt, ctx_save->mailbox);
      if (rc != 0)
        break;
#ifdef USE_COMPRESSED
      if (m_comp)
