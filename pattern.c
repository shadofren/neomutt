                      ...
                      struct Mailbox *m, struct Email *e, struct PatternCache *cache)
{
  ...
  {
    case MUTT_PAT_AND:
      return pat->not_^(perform_and(pat->child, flags, m, e, cache) > 0);
    case MUTT_PAT_OR:
      return pat->not_^(perform_or(pat->child, flags, m, e, cache) > 0);
    case MUTT_PAT_THREAD:
      return pat->not_^match_threadcomplete(pat->child, flags, m, e->thread, 1, 1, 1, 1);
    case MUTT_PAT_PARENT:
      return pat->not_^match_threadparent(pat->child, flags, m, e->thread);
    case MUTT_PAT_CHILDREN:
      return pat->not_^match_threadchildren(pat->child, flags, m, e->thread);
    case MUTT_ALL:
      return !pat->not_;
    case MUTT_EXPIRED:
      return pat->not_^e->expired;
    case MUTT_SUPERSEDED:
      return pat->not_^e->superseded;
    case MUTT_FLAG:
      return pat->not_^e->flagged;
    case MUTT_TAG:
      return pat->not_^e->tagged;
    case MUTT_NEW:
      return pat->not_? e->old || e->read : !(e->old || e->read);
    case MUTT_UNREAD:
      return pat->not_? e->read : !e->read;
    case MUTT_REPLIED:
      return pat->not_^e->replied;
    case MUTT_OLD:
      return pat->not_? (!e->old || e->read) : (e->old && !e->read);
    case MUTT_READ:
      return pat->not_^e->read;
    case MUTT_DELETED:
      return pat->not_^e->deleted;
    case MUTT_PAT_MESSAGE:
      return pat->not_^((EMSG(e) >= pat->min) && (EMSG(e) <= pat->max));
    case MUTT_PAT_DATE:
      if (pat->dynamic)
        match_update_dynamic_date(pat);
      return pat->not_^(e->date_sent >= pat->min && e->date_sent <= pat->max);
    case MUTT_PAT_DATE_RECEIVED:
      if (pat->dynamic)
        match_update_dynamic_date(pat);
      return pat->not_^(e->received >= pat->min && e->received <= pat->max);
    case MUTT_PAT_BODY:
    ...
    case MUTT_PAT_WHOLE_MSG:
      ...
      if ((m->magic == MUTT_IMAP) && pat->stringmatch)
        return e->matched;
#endif
      return pat->not_^msg_search(m, pat, e->msgno);
    ...
    case MUTT_PAT_SENDER:
      if (!e->env)
        return 0;
      return pat->not_^match_addrlist(pat, (flags & MUTT_MATCH_FULL_ADDRESS), 1,
                                     &e->env->sender);
    case MUTT_PAT_FROM:
      if (!e->env)
        return 0;
    ...
    case MUTT_PAT_SUBJECT:
      if (!e->env)
        return 0;
      return pat->not_^(e->env->subject &&patmatch(pat, e->env->subject));
    case MUTT_PAT_ID:
    case MUTT_PAT_ID_EXTERNAL:
      if (!e->env)
        return 0;
      return pat->not_^(e->env->message_id &&patmatch(pat, e->env->message_id));
    case MUTT_PAT_SCORE:
      return pat->not_^(e->score >= pat->min &&
                       (pat->max == MUTT_MAXRANGE || e->score <= pat->max));
    case MUTT_PAT_SIZE:
      return pat->not_^(e->content->length >= pat->min &&
                       (pat->max == MUTT_MAXRANGE || e->content->length <= pat->max));
    case MUTT_PAT_REFERENCE:
      if (!e->env)
        return 0;
      return pat->not_^(match_reference(pat, &e->env->references) ||
                       match_reference(pat, &e->env->in_reply_to));
    case MUTT_PAT_ADDRESS:
      if (!e->env)
        return 0;
      return pat->not_^match_addrlist(pat, (flags & MUTT_MATCH_FULL_ADDRESS), 4,
                                     &e->env->from, &e->env->sender,
                                     &e->env->to, &e->env->cc);
    case MUTT_PAT_RECIPIENT:
      if (!e->env)
        return 0;
      return pat->not_^match_addrlist(pat, (flags & MUTT_MATCH_FULL_ADDRESS), 2,
                                     &e->env->to, &e->env->cc);
    ...
    case MUTT_PAT_COLLAPSED:
      return pat->not_^(e->collapsed && e->num_hidden > 1);
    case MUTT_PAT_CRYPT_SIGN:
      if (!WithCrypto)
        break;
      return pat->not_^((e->security & SEC_SIGN) ? 1 : 0);
    case MUTT_PAT_CRYPT_VERIFIED:
      if (!WithCrypto)
        break;
      return pat->not_^((e->security & SEC_GOODSIGN) ? 1 : 0);
    case MUTT_PAT_CRYPT_ENCRYPT:
      if (!WithCrypto)
        break;
      return pat->not_^((e->security & SEC_ENCRYPT) ? 1 : 0);
    case MUTT_PAT_PGP_KEY:
      if (!(WithCrypto & APPLICATION_PGP))
        break;
      return pat->not_^((e->security & PGP_KEY) == PGP_KEY);
    case MUTT_PAT_XLABEL:
      if (!e->env)
        return 0;
      return pat->not_^(e->env->x_label &&patmatch(pat, e->env->x_label));
    case MUTT_PAT_DRIVER_TAGS:
    {
      char *tags = driver_tags_get(&e->tags);
      bool rc = (pat->not_^(tags &&patmatch(pat, tags)));
      FREE(&tags);
      return rc;
    }
    case MUTT_PAT_HORMEL:
      if (!e->env)
        return 0;
      return pat->not_^(e->env->spam && e->env->spam->data &&
                       patmatch(pat, e->env->spam->data));
    case MUTT_PAT_DUPLICATED:
      return pat->not_^(e->thread && e->thread->duplicate_thread);
    case MUTT_PAT_MIMEATTACH:
      if (!m)
        return 0;
      {
        int count = mutt_count_body_parts(m, e);
        return pat->not_^(count >= pat->min &&
                         (pat->max == MUTT_MAXRANGE || count <= pat->max));
      }
    case MUTT_PAT_MIMETYPE:
      if (!m)
        return 0;
      return pat->not_^match_mime_content_type(pat, m, e);
    case MUTT_PAT_UNREFERENCED:
      return pat->not_^(e->thread && !e->thread->child);
    case MUTT_PAT_BROKEN:
      return pat->not_^(e->thread && e->thread->fake_thread);
#ifdef USE_NNTP
    case MUTT_PAT_NEWSGROUPS:
      if (!e->env)
        return 0;
      return pat->not_^(e->env->newsgroups &&patmatch(pat, e->env->newsgroups));
#endif
  }
...
bool mutt_limit_current_thread(struct Email *e)
{
  ...
  {
    Context->mailbox->emails[i]->virtual_ = -1;
    ...
    if (top_of_thread(Context->mailbox->emails[i]) == me)
    {
      ...
      Context->mailbox->emails[i]->virtual_ = Context->mailbox->vcount;
